...
<template>
  ...
  <ContentWrap>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="编号" min-width="140" prop="id" />
      ...
      <el-table-column label="仓储类型" min-width="140" prop="storageType">
        <template #default="{ row }">
          <div>
            {{ccType[row.storageType]? ccType[row.storageType].name:'-' }}
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="商品重量(计算运费的重量)" min-width="120" prop="spuWeight" />

      <el-table-column align="center" label="商品体积(计算运费的体积)" min-width="120" prop="spuVolume" />

      <el-table-column align="center" label="商品来源链接" min-width="120" prop="sourceUrl">
        <template #default="{ row }">
          <div :class="row">
            <!-- {{ row.sourceUrl }} -->
            <el-image :src="row.sourceUrl" class="flex-none w-50px h-50px" />
            <!-- <el-image fit="cover"
              src="http://img.doutupk.com/production/uploads/image/2024/08/26/20240826640461_EsXCed.jpg"
