...
<template>
  <el-form ref="formRef" v-loading="formLoading" :disabled="isDetail" :model="formData" :rules="rules"
    ...
    <template v-if="formData.specType && propertyList.length > 0">
      ...
      <el-form-item v-if="!isDetail" label="单独列设置">
        <SkuSettingList :updateOneSelectVal="updateOneSelectVal" :propertyList="propertyList" ref="skuSettingListRef"
          style="flex: 1;" />
        <!-- <SkuList :is-batch="true" :prop-form-data="formData" :property-list="propertyList" /> -->
      </el-form-item>
...
import SkuSettingList from '@/views/mall/product/spu/components/skuSettingList.vue'
...
// const findMatchingIndex = (array, targetProperties) => {
//   return array.findIndex((item) => {
//     // 将 targetProperties 转换为一个 Map，以便快速查找
//     const targetMap = new Map(targetProperties.map((prop) => [prop.propertyId, prop]))

//     // 检查 item.properties 是否与 targetProperties 匹配
//     return (
//       item.properties.length === targetProperties.length &&
//       item.properties.every((prop) => {
//         const targetProp = targetMap.get(prop.propertyId)
//         return (
//           targetProp &&
//           prop.propertyName === targetProp.propertyName &&
//           prop.valueId === targetProp.valueId &&
//           prop.propertyId === targetProp.propertyId
//         )
//       })
//     )
//   })
// }
function findMatchingIndices(array, targetProperties) {
  return array.reduce((matches, item, index) => {
    if (
      targetProperties.every((targetProp) =>
        item.properties.some(
          (prop) =>
            prop.propertyId === targetProp.propertyId &&
            prop.valueId === targetProp.valueId &&
            (targetProp.name === undefined || prop.name === targetProp.name)
        )
      )
    ) {
      matches.push(index)
...
// 单独修改方法
const updateOneSelectVal = (val) => {
  val.selectedArr.forEach((item) => {
    let indexs = findMatchingIndices(formData.skus, item)
    indexs.forEach((index) => {
      for (let key in val.valuesObj) {
        formData.skus[index][key] = val.valuesObj[key]
      }
    })
  })
}
