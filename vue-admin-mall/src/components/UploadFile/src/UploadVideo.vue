<template>
  <div v-if="!disabled" class="upload-file">
    <el-upload :headers="{
      'tenant-id':'1'
    }" ref="uploadRef" v-model:file-list="fileList" :action="uploadUrl" :auto-upload="autoUpload"
      :before-upload="beforeUpload" :disabled="disabled" :drag="drag" :limit="props.limit" :multiple="props.limit > 1"
      :on-error="excelUploadError" :on-exceed="handleExceed" :on-preview="handlePreview" :on-remove="handleRemove"
      :on-success="handleFileSuccess" :show-file-list="true" class="upload-file-uploader" name="file">
      ...
      <template #file="row">
        <div class="flex items-center">
          ...
          <div v-if="row.file.url">
            <vue3VideoPlay v-bind="VideoOptions" :src="row.file.url" poster="row.file.url" />
          </div>
...
const props = defineProps({
  ...
  fileType: propTypes.array.def(['mp4', 'mov', 'avi']), // 文件类型, 例如['png', 'jpg', 'jpeg']
